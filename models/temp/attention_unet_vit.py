...
from monai.networks.nets.basic_unet import BasicUNet, TwoConv, UpCat
    ...
    def __init__(
        ...
        if use_abs_pos:
            ...
            self.pos_embed = nn.Parameter(
                torch.zeros(1, img_depth // patch_size, img_size // patch_size, img_size // patch_size, embed_dim)
            )
        ...
        for i in range(depth):
            block = Block(
                ...
                input_size=(img_depth // patch_size, img_size // patch_size, img_size // patch_size),
            )
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        if spatial_dims == 2:
            ...
            raise Exception("spatial_dims must be 2 or 3.")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.ndim == 4:
            ...
            raise Exception(f"X has {x.ndim} dims, expected 4 or 5.")
...
def window_unpartition(
    ...
    Dp, Hp, Wp = pad_dhw
    D, H, W = dhw
    B = windows.shape[0] // (Dp * Hp * Wp // window_size // window_size)
    x = windows.view(B, Dp // window_size, Hp // window_size, Wp // window_size, window_size, window_size, window_size, -1)
    x = x.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous().view(B, Dp, Hp, Wp, -1)
...
def add_decomposed_rel_pos(
    ...
    print(f"rel_d : {rel_d.shape}")
    print(f"rel_h : {rel_h.shape}")
    print(f"rel_w : {rel_w.shape}")
